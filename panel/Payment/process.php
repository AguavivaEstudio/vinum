...
<body>
    ...
    <?php
			...
			if (($permissions['view'] != 1) || (is_null($table))) {
				...
				$sSql = "SELECT `id`, `fileName` FROM `sys_files` WHERE `tableName` = 'precios_archivos' AND `columnName` = 'file_clientes' AND `publish` = 1;";
				$result = ExecuteSql($sSql, null);
				$sSql = "TRUNCATE TABLE `precios_import`;";
				...
				while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
					...
					$rowsAffected = importFile($baseDir . $fileName, 'precios_import');
				...
				if ($archivosProcesados > 0) {
					...
					$sSql = "
						INSERT INTO `publicaciones` (`titulo`, `editorial`, `precio`, `datosedic`,`autor`, `autor2`, `autor3`,`autor4`,`autor5`,`isbn`,`ano`,`especialidad`, `codigo`, `activo`)
							SELECT CI.`titulo`, 1, CI.`precio`, NULL, 1, 1, 1, 1, 1, NULL, NULL, 1, CI.`codigo`, 0
						  	FROM `precios_import` AS CI
					...
					$sSql = "
							UPDATE `publicaciones` AS CL
							 INNER JOIN `precios_import` AS CI ON CL.`codigo` = CI.`codigo`
