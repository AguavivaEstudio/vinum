...
<body>
	...
	if (($permissions['view'] != 1) || (is_null($table))){
		...
		echo "<thead><tr><th></th><th>Tipo</th><th>Título</th><th>Publicado</th><th>Órden</th><th>(ES) Imagenes</th><th>(EN) Imagenes</th></tr></thead>";
		echo "<tbody>";
		...
		while ($row = $result -> fetch_array(MYSQLI_ASSOC)) {
			...
			echo $row['name'];
			echo "</td>";
			echo "<td class='text-center'>" . CreateCheckBox($table2, 'active', $row['id'], $row['active'], $disabled, 'clsPublish') . "</td>";
			echo "<td>";
			echo $row['order'];
			echo "</td>";
			echo "<td class='text-center'>";
			if (($row['type'] == 'image') || ($row['type'] == 'carrusel')) { 
				echo "<i class='fa fa-picture-o' data-table='".$table2."' data-column='file_images' data-id='" . $row['id'] . "' aria-hidden='false'></i>";
			}
			echo "</td>";
			echo "<td class='text-center'>";
			if (($row['type'] == 'image') || ($row['type'] == 'carrusel')) { 
				echo "<i class='fa fa-picture-o' data-table='".$table2."' data-column='file_images_en' data-id='" . $row['id'] . "' aria-hidden='false'></i>";
			}
			echo "</td>";
			echo "</tr>";
		}
